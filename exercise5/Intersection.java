...
public class Intersection extends Actor {
	...
	public double setName(double name) {
		return name;
	}
	...
	public void addNSLights(int nRotation, int sRotation) {
		...
		getWorld().addObject(sTL, this.getX(),
				this.getY() + TrafficWorld.ROAD_LENGTH / 2 + sTL.getImage().getHeight() / 2);
		sTL.setRotation(sRotation);
		nTL = new TrafficLight(verticalColor);
		getWorld().addObject(nTL, this.getX(),
				this.getY() - TrafficWorld.ROAD_LENGTH / 2 - nTL.getImage().getHeight() / 2);
	...
	public void addEWLights(int eRotation, int wRotation) {
		...
		getWorld().addObject(eTL, this.getX() + TrafficWorld.ROAD_LENGTH / 2 + eTL.getImage().getHeight() / 2,
				this.getY());
		eTL.setRotation(eRotation);
		wTL = new TrafficLight(horizontalColor);
		getWorld().addObject(wTL, this.getX() - TrafficWorld.ROAD_LENGTH / 2 - wTL.getImage().getHeight() / 2,
				this.getY());
	...
	public void notifyInCars() {
		...
		for (Car c : current) {
			c.isIn(this);
			startTimeMillis();
			data.time(c, startTime - arrivalTime);
			data.carsInIntersection(TrafficWorld.intersections());
			left = false;	
		}
		prev = current;
	}
